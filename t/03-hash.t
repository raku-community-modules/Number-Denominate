...
is-deeply %data, { hours => 3, minutes => 31, second => 1 },
...
is-deeply denominate( 12661, :hash,
    ...
    ), { meow => 3, beers => 31, foo => 1 },
    'testing "s"-less units';

is-deeply denominate( 12660, :hash, :set<time> ),
    { hours => 3, minutes => 31 },
    'testing "missing" units, when their number is 0 [test 1]';

is-deeply denominate( 3*3600, :hash, :set<time> ), { hours => 3 },
...
is-deeply denominate( 3, :hash, :set<time> ), { seconds => 3 },
